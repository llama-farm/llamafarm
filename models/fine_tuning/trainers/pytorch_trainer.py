...
class PyTorchFineTuner(BaseFineTuner):
    """PyTorch-based fine-tuning implementation."""
    ...
    def validate_config(self) -> List[str]:
        ...
        if "huggingface_id" not in self.config.base_model:
        ...
        if method_type in ["lora", "qlora"]:
            if "r" not in self.config.method:
                errors.append("LoRA rank 'r' is required for LoRA methods")
            if "alpha" not in self.config.method:
        ...
        if "path" not in self.config.dataset and "dataset_name" not in self.config.dataset:
    ...
    def prepare_model(self) -> Any:
        ...
        model_config = self.config.base_model
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            ...
            use_fast=self.config.framework.get("use_fast_tokenizer", True)
        )
        ...
        model_kwargs = {
            ...
            "low_cpu_mem_usage": self.config.environment.get("low_cpu_mem_usage", True)
        }
    ...
    def prepare_dataset(self) -> Any:
        ...
        if "path" in dataset_config:
            ...
            data_format = dataset_config.get("data_format", "jsonl")
            
            # Use data processor to handle dataset
            processor = DataProcessorFactory.create_from_format(data_format, dataset_config)
            self.dataset = processor.process_dataset(dataset_path)
    ...
    def _tokenize_dataset(self, dataset) -> Dataset:
        ...
        max_length = self.config.training_args.get("max_seq_length", 512)
        ...
        tokenized_dataset = dataset.map(
            tokenize_function,
            batched=True,
            num_proc=self.config.dataset.get("preprocessing_num_workers", 4),
    ...
    def _create_training_arguments(self) -> TrainingArguments:
        ...
        config = self.config.training_args
        ...
        args = TrainingArguments(
            ...
            gradient_checkpointing=self.config.framework.get("gradient_checkpointing", True),
        )
