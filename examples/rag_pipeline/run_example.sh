...
./lf run "What is transformer architecture? Give a brief answer."
echo ""

echo -e "${YELLOW}Query 2: With RAG enabled${NC}"
echo "Question: What is transformer architecture?"
./lf run --rag --database main_database "What is transformer architecture? Give a brief answer with specific details."
echo ""

echo -e "${YELLOW}Query 3: Code-related query with RAG${NC}"
echo "Question: What is the DataProcessor class?"
./lf run --rag --database main_database "What is the DataProcessor class and what methods does it have?"
echo ""

echo -e "${YELLOW}Query 4: Research paper query with custom top-k${NC}"
echo "Question: Explain neural scaling laws"
./lf run --rag --database main_database --rag-top-k 5 "Explain neural scaling laws and their implications"
echo ""
...
echo "  ./lf run --help        # See all query options"
