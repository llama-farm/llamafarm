#!/usr/bin/env bash
# Quick RAG demo (small engineering notes)
...
EXAMPLE_NAME="quick"
BASE_DATABASE_NAME="quick_rag_db"
PROCESSOR_NAME="quick_note_processor"
...
ensure_file "$CONFIG_PATH" "No llamafarm.yaml found at ${CONFIG_PATH}. Run './examples/quick-rag/update_config.sh' first."
ensure_dir "$FILES_DIR" "Sample notes directory missing: ${FILES_DIR}."
...
bold "Step 3: Upload engineering notes"
...
bold "Step 4: Process dataset"
"${LF_BIN}" datasets process "${DATASET_NAME}"
success "Processing complete."
pause

bold "Step 5: Ask quick questions"
"${LF_BIN}" chat --database "${DATABASE_NAME}" \
  "Summarize the neural scaling laws note in two sentences with citations."

pause
"${LF_BIN}" chat --database "${DATABASE_NAME}" \
  "List any best practices mentioned for API design. Cite the source file."
...
bold "Step 6: Compare baseline"
"${LF_BIN}" chat --no-rag "Summarize neural scaling laws in two sentences."

pause
success "Quick demo complete. Continue exploring with 'lf chat --database ${DATABASE_NAME} "YOUR QUESTION"'."
warn "Database '${DATABASE_NAME}' and dataset '${DATASET_NAME}' remain available. Clean up when finished."
