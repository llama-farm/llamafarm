...
class GGUFLanguageModel(BaseModel):
    ...
    def __init__(
        ...
        # Validate the alias before we store it — it will be used to
        # construct filesystem paths under LLAMAFARM_MODEL_DIR later, so
        # a traversal-bearing alias must be rejected at construction time.
    ...
    async def load(self) -> None:
        ...
        if mmproj_path is None and self.auto_detect_mmproj and not is_local_file:
            try:
                # Prefer the four-tier resolver when an alias is known so
                # LLAMAFARM_MODEL_DIR can supply the mmproj file alongside
                # the main weights. Fall back to the legacy entry point
                # (which still honors offline mode) when no alias is set.
                if self.alias:
