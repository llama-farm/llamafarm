...
export function DetectionFeed({ detections, alerts, selectedDrone, collapsed, onToggleCollapsed, onDetectionClick, onAlertAction, onAlertDismiss, onOpenStream, layout = 'mobile', hideStream = false }: DetectionFeedProps) {
        ...
        feedItems.map(item => {
          if (item.type === 'alert' && item.alert) {
            ...
            return (
              <div
                key={item.id}
                className={`w-full px-2.5 py-1.5 text-left border-b border-surface-border/50 border-l-2 ${priority.border} ${priority.bg} animate-fade-in`}
              >
          ...
          if (item.type === 'detection' && item.detection) {
            return (
              <button
                ...
                className="w-full px-2.5 py-1.5 text-left hover:bg-white/5 border-b border-surface-border/50 transition-colors animate-fade-in"
              >
                <div className="flex items-center gap-1.5 mb-0.5">
                  <div
                    className="w-2 h-2 rounded-full shrink-0 animate-pulse-detection"
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-3 py-2 border-b border-surface-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-text-secondary tracking-wide">Detection Feed</span>
            {activeAlerts > 0 && (
              <span className="w-4 h-4 rounded-full bg-status-critical flex items-center justify-center text-[10px] text-white font-bold animate-pulse">
                {activeAlerts}
              </span>
            )}
          </div>
          <span className="text-[10px] text-text-dim">{feedItems.length} items</span>
        </div>
        ...
        {!hideStream && isStreaming && selectedDrone && (
          <div className="border-t border-surface-border shrink-0">
            ...
            {!streamCollapsed && (
              <div className="relative w-full aspect-[16/9] bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
                ...
                {(selectedDrone.status === 'flying' || selectedDrone.status === 'returning') ? (
                  ...
                  <div className="absolute top-1.5 left-1.5 flex items-center gap-1 bg-black/60 px-1.5 py-0.5 rounded text-[10px] font-bold text-text-dim">STANDBY</div>
                )}
                <div className="absolute top-1.5 right-1.5 bg-black/60 px-1.5 py-0.5 rounded text-[10px] text-text-primary">{selectedDrone.name}</div>
              </div>
            )}
          </div>
        )}
  ...
  // ===== MOBILE LAYOUT (original absolute overlay) =====
  return (
