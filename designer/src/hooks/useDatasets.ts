  ...
  return useMutation({
    mutationFn: (
      ...
      datasetService.createDataset(data.namespace, data.project, {
        ...
        document_scanning: data.document_scanning,
      }),
...
/**
 * Hook to delete chunks for a file from the vector store (without deleting the source file)
 * Used for reprocessing files
 * @returns Mutation for deleting file chunks
 */
export function useDeleteFileChunks() {
  return useMutation({
    mutationFn: (data: {
      namespace: string
      project: string
      ...
      datasetService.deleteFileChunks(
        data.namespace,
        data.project,
        data.dataset,
        data.fileHash
