...
/**
 * Docling Parser: AI-powered document parsing using IBM's Docling library.
 * Features: Layout analysis (DocLayNet), Table extraction (TableFormer),
 * Smart chunking (HybridChunker), OCR support, multi-format support.
 * Recommended as the PRIMARY parser for PDFs and complex documents.
 *
 */
export interface DoclingParserConfig {
  /**
   * Target chunk size in tokens (uses HybridChunker tokenizer)
   */
  chunk_size?: number;
  /**
  ...
  /**
   * Chunking strategy:
   * - hybrid: Tokenizer-aware chunking that respects document structure (recommended)
   * - hierarchical: One chunk per document element
   * - none: No chunking, return full document
   *
   */
  chunk_strategy?: 'hybrid' | 'hierarchical' | 'none';
  /**
   * Extract document metadata (headings, page count, tables)
   */
  extract_metadata?: boolean;
  /**
...
/**
 * MarkItDown Parser: Fast document-to-markdown conversion using Microsoft's library.
 * Features: Quick conversion, wide format support, minimal dependencies.
 * Use for: Simple documents, fast conversion when AI analysis isn't needed.
 * Note: Does not include chunking - use SimpleChunker separately if needed.
 *
 */
export interface MarkItDownParserConfig {
  /**
   * Output content format
   */
  ...
  extract_metadata?: boolean;
  /**
   * Use LLM for image description (requires API key)
   */
  use_llm_for_images?: boolean;
  /**
   * LLM model for image description (if use_llm_for_images is true)
   */
  llm_model?: string;
}
  ...
  "DoclingParser_": {
    "properties": {
      "chunk_size": {
        ...
        "minimum": 50,
        "maximum": 8192,
        "description": "Target chunk size in tokens (uses HybridChunker tokenizer)"
      },
      "chunk_overlap": {
        "type": "integer",
        "default": 50,
      ...
      "extract_metadata": {
        "type": "boolean",
        "default": true,
        "description": "Extract document metadata (headings, page count, tables)"
      },
      "extract_tables": {
        "type": "boolean",
        "default": true,
      ...
      "preserve_structure": {
        "type": "boolean",
        "default": true,
        "description": "Preserve document structure in output"
      },
      "page_aware": {
        "type": "boolean",
        "default": true,
        "description": "Include page numbers in chunk metadata"
      }
    },
    "title": "doclingParserConfig",
    "description": "Docling Parser: AI-powered document parsing using IBM's Docling library.\nFeatures: Layout analysis (DocLayNet), Table extraction (TableFormer),\nSmart chunking (HybridChunker), OCR support, multi-format support.\nRecommended as the PRIMARY parser for PDFs and complex documents.\n",
    "defaultExtensions": [
      ".pdf",
      ".docx",
      ".pptx",
  ...
  "MarkItDownParser_": {
    "properties": {
      ...
      "use_llm_for_images": {
        "type": "boolean",
        "default": false,
        "description": "Use LLM for image description (requires API key)"
      },
      "llm_model": {
        "type": "string",
        "default": "gpt-4o-mini",
        "description": "LLM model for image description (if use_llm_for_images is true)"
      }
    },
    "title": "markItDownParserConfig",
    "description": "MarkItDown Parser: Fast document-to-markdown conversion using Microsoft's library.\nFeatures: Quick conversion, wide format support, minimal dependencies.\nUse for: Simple documents, fast conversion when AI analysis isn't needed.\nNote: Does not include chunking - use SimpleChunker separately if needed.\n",
    "defaultExtensions": [
      ".pdf",
      ".docx",
      ".xlsx",
