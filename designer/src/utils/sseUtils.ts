...
export async function parseSSEStream<T = any>(
  ...
  try {
    ...
    const parseError = error instanceof Error ? error : new Error('Unknown SSE parsing error')
    onError?.(parseError)
    throw parseError
  } finally {
    reader.releaseLock()
 ...
 */
export function getSSEReader(response: Response): ReadableStreamDefaultReader<Uint8Array> {
