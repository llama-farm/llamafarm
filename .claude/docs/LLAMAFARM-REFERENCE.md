...
All 12 backends are powered by PyOD. Use `GET /v1/ml/anomaly/backends` to list all with metadata.

| Backend | Category | Best For |
|---------|----------|----------|
| `isolation_forest` | Legacy | General purpose, fast (recommended default) |
| `one_class_svm` | Legacy | Tight clusters, clear boundaries |
| `local_outlier_factor` | Legacy | Varying densities, local patterns |
| `autoencoder` | Deep Learning | Complex patterns, neural approach |
| `ecod` | Fast | **Recommended for new projects** - parameter-free |
| `hbos` | Fast | Fastest algorithm, high dimensions |
| `copod` | Fast | Parameter-free, interpretable |
| `knn` | Distance | When density matters |
| `mcd` | Distance | Gaussian-distributed data |
| `cblof` | Clustering | Clustered data |
| `suod` | Ensemble | Most robust, combines multiple algorithms |
| `loda` | Streaming | Online/streaming scenarios |
...
For real-time data streams, use the streaming API that handles cold start, auto-retraining, and sliding windows.

**Endpoint:** `POST /v1/ml/anomaly/stream`
...
**Response Statuses:**
- `collecting`: Cold start phase
- `ready`: Model trained, returning scores
- `retraining`: Background retrain in progress
...
# Get detector stats
curl http://localhost:8005/v1/ml/anomaly/stream/{model_id}

# Reset detector
curl -X POST http://localhost:8005/v1/ml/anomaly/stream/{model_id}/reset

# Delete detector
curl -X DELETE http://localhost:8005/v1/ml/anomaly/stream/{model_id}
```

### Polars Buffer API

Direct access to Polars-based data buffers for advanced use cases.

**Endpoints:**
| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/v1/ml/polars/buffers` | POST | Create buffer |
| `/v1/ml/polars/buffers` | GET | List buffers |
| `/v1/ml/polars/buffers/{id}` | GET | Get buffer stats |
| `/v1/ml/polars/buffers/{id}` | DELETE | Delete buffer |
| `/v1/ml/polars/buffers/{id}/clear` | POST | Clear buffer data |
| `/v1/ml/polars/append` | POST | Append data |
| `/v1/ml/polars/features` | POST | Compute rolling features |
| `/v1/ml/polars/buffers/{id}/data` | GET | Get raw data |
...
curl -X POST http://localhost:8005/v1/ml/polars/append \
  ...
  -d '{
    ...
    "data": [{"temp": 72.5, "humidity": 45}, {"temp": 73.1, "humidity": 44}]
  }'
```
...
Returns columns like: `temp_rolling_mean_5`, `temp_rolling_std_10`, `temp_lag_1`
