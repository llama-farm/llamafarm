...
func (m *UVManager) EnsureUV() (string, error) {
	...
	if m.isUVInstalled() {
		utils.LogDebug("UV already installed at: " + m.uvPath)
		return m.uvPath, nil
	}
	...
	utils.LogDebug(fmt.Sprintf("UV installed successfully to %s", m.binDir))
	return m.uvPath, nil
}
...
func (m *UVManager) downloadUV() error {
	...
	utils.LogDebug(fmt.Sprintf("Downloading UV %s for %s/%s...", uvVersion, runtime.GOOS, runtime.GOARCH))
