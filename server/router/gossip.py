...
class CapabilityInfo:
    ...
    label: str
    description: str
    vector: List[float]  # Serialized from np.ndarray
    local: bool = True
    hops: int = 0
...
class ResourceInfo:
    ...
    cpu_available: float = 1.0  # 0-1
    memory_available_mb: int = 0
    gpu_available: float = 0.0  # 0-1
...
class GossipProtocol:
    ...
    announcements and updates gradient table. Forwards announcements
    ...
    Usage:
        gossip = GossipProtocol(
            ...
            local_capabilities=my_capabilities
        )

        # Set up broadcast mechanism
        gossip.set_broadcast_callback(send_to_all_peers)

        # Start periodic announcements
        await gossip.start()

        # Handle incoming announcement
        await gossip.handle_announcement(msg_data, from_peer="peer-1")
    ...
    def __init__(
        ...
        self._nonce_cache: Dict[str, float] = {}  # nonce -> timestamp
        self._nonce_cache_lock = asyncio.Lock()

        # Track which nodes we've heard from
        self._known_nodes: Dict[str, float] = {}  # node_id -> last_seen
    ...
    def get_resource_info(self) -> ResourceInfo:
        """Get current resource usage. Override for actual monitoring."""
        # Placeholder - in real implementation, query system resources
        return ResourceInfo(
            cpu_available=0.7,
    ...
    def build_announcement(self) -> Announcement:
        """Build an announcement message with local + forwarded capabilities."""
        capabilities = []
        ...
        # Add forwarded capabilities from gradient table
    ...
    async def announce(self) -> None:
        ...
        if not self._broadcast_callback:
            logger.warning("No broadcast callback set, cannot announce")
            return
        ...
        try:
            ...
            logger.debug(
                f"Announced {len(announcement.capabilities)} capabilities"
            )
    ...
    async def handle_announcement(
        ...
        Updates gradient table and optionally forwards to other peers.

        Args:
            data: Raw announcement message
            from_peer: Node ID of the sender
            forward_callback: Optional callback to forward to other peers
        """
        try:
        ...
        if not await self._check_nonce(announcement.nonce, announcement.timestamp):
            logger.debug(f"Replay detected from {from_peer}, ignoring")
            return
        ...
        for cap in announcement.capabilities:
            ...
            updated = self.gradient_table.update(
                ...
                estimated_latency_ms=cap.estimated_latency_ms + 10  # Add link latency
            )
            if updated:
                updates += 1
        ...
        if updates > 0:
            logger.debug(
                f"Updated {updates} gradient entries from {announcement.from_node}"
            )
...
class AnnouncementBuilder:
    """Helper for building capability announcements."""

    def __init__(self, node_id: str):
        self.node_id = node_id
        self.capabilities: List[CapabilityInfo] = []
        self.resources: Optional[ResourceInfo] = None
