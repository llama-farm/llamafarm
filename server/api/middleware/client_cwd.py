...
current_client_cwd: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "current_client_cwd", default=None
)
...
class ClientCWDHeaderMiddleware(BaseHTTPMiddleware):
    ...
    the `current_client_cwd` context variable.
    """
        ...
        if not settings.lf_use_data_dir:
            ...
            if header_value:
                ...
                try:
                    ...
                    current_client_cwd.reset(token)
                return response
        # Default path
