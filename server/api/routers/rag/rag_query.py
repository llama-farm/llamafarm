...
from rag.core.factories import VectorStoreFactory, EmbedderFactory, RetrievalStrategyFactory
...
async def handle_rag_query(
    ...
    if not database_name:
        raise HTTPException(status_code=400, detail="No database specified and no default available")
    ...
    if not retrieval_strategy_name:
        ...
        if not retrieval_strategy_name:
            ...
            if database_config.retrieval_strategies:
                retrieval_strategy_name = database_config.retrieval_strategies[0].name
    
    if not retrieval_strategy_name:
        raise HTTPException(status_code=400, detail="No retrieval strategy specified and no default available")
    ...
    if not retrieval_config:
        raise HTTPException(status_code=404, detail=f"Retrieval strategy '{retrieval_strategy_name}' not found")
    ...
    if not embedder_config:
        raise HTTPException(status_code=500, detail=f"Embedding strategy '{embedder_name}' not found")
    ...
    try:
        ...
        if 'persist_directory' in store_config and not store_config['persist_directory'].startswith('/'):
            # Make persist_directory absolute based on project_dir
            from pathlib import Path
            store_config['persist_directory'] = str(Path(project_dir) / store_config['persist_directory'])
            logger.info(f"Resolved persist_directory to: {store_config['persist_directory']}")
        
        store = VectorStoreFactory.create(
            component_type=store_type,
            config=store_config
        )
        
        # Initialize retriever without store in config (it's passed as a parameter)
        retriever_config = retrieval_config.config.copy() if retrieval_config.config else {}
        ...
        retriever = RetrievalStrategyFactory.create(
            component_type=retriever_type,
            config=retriever_config
        )
        
        # Perform search - pass vector_store as parameter
        logger.info(f"Searching with strategy '{retrieval_strategy_name}' in database '{database_name}'")
