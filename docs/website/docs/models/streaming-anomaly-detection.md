...
curl http://localhost:8005/v1/ml/anomaly/backends
```
...
```bash
curl http://localhost:8005/v1/ml/anomaly/stream/sensor-stream
```
...
```bash
curl -X POST http://localhost:8005/v1/ml/anomaly/stream/sensor-stream/reset
```
...
```bash
curl -X DELETE http://localhost:8005/v1/ml/anomaly/stream/sensor-stream
```
...
async def stream_sensor_data():
    ...
    async with httpx.AsyncClient() as client:
        base_url = "http://localhost:8005/v1/ml/anomaly/stream"
...
async def advanced_streaming():
    async with httpx.AsyncClient() as client:
        base_url = "http://localhost:8005/v1/ml"
...
curl -X POST http://localhost:8005/v1/ml/anomaly/stream/my-detector/reset
```
