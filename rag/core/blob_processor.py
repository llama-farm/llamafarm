...
from rag.components.parsers.base.base_parser import BaseParser
...
class BlobProcessor:
    ...
    def __init__(self, strategy_config: Dict[str, Any]):
        ...
        self.extractors = self._initialize_extractors(strategy_config.get("extractors", []))
        
    def _initialize_parsers(self, parser_configs: List[Dict[str, Any]]) -> List[Tuple[Dict, BaseParser]]:
        """
    ...
    def _initialize_extractors(self, extractor_configs: List[Dict[str, Any]]) -> List[Tuple[Dict, Any]]:
        """
    ...
    def _get_parser_class(self, parser_type: str) -> type:
        ...
        if '_' in parser_type:
            ...
            if 'Parser' in parts[0]:
                ...
                implementation = '_'.join(parts[2:]).lower() if len(parts) > 2 else 'default'
        else:
        ...
        if parsers_dir.exists():
            ...
            for category_dir in parsers_dir.iterdir():
                if category_dir.is_dir() and parser_category in category_dir.name.lower():
        ...
        logger.warning(f"Could not dynamically load parser {parser_type}, using mock parser")
        ...
        class MockParser(BaseParser):
            ...
            def _load_metadata(self):
                from rag.components.parsers.base.base_parser import ParserConfig
                return ParserConfig(
                    name=parser_type,
    ...
    def _get_extractor_class(self, extractor_type: str) -> type:
        ...
        if extractor_type == "ContentStatisticsExtractor":
            potential_paths.insert(0, "rag.components.extractors.statistics_extractor.statistics_extractor")
        elif extractor_type == "EntityExtractor":
            potential_paths.insert(0, "rag.components.extractors.entity_extractor.entity_extractor")
        elif extractor_type == "KeywordExtractor":
            potential_paths.insert(0, "rag.components.extractors.keyword_extractor.keyword_extractor")
        ...
        for module_path in potential_paths:
            try:
                ...
                if hasattr(module, extractor_type):
                    ...
                    logger.debug(f"Successfully loaded {extractor_type} from {module_path}")
        ...
        logger.warning(f"Could not dynamically load extractor {extractor_type}, using mock extractor")
    ...
    def _is_excluded(self, filename: str, exclude_patterns: List[str]) -> bool:
        ...
        """
        return self._matches_patterns(filename, exclude_patterns) if exclude_patterns else False
    
    def process_blob(self, blob_data: bytes, metadata: Dict[str, Any]) -> List[Document]:
        """
    ...
    def _apply_extractors(self, documents: List[Document], filename: str) -> List[Document]:
        """
    ...
    def _find_matching_extractors(self, filename: str) -> List[Tuple[Dict, BaseExtractor]]:
        """
