    ...
    for comp_type in component_types:
        ...
        for comp_dir in type_dir.iterdir():
            ...
            if schema_path.exists():
                ...
                defaults = (
                    load_yaml_file(defaults_path) if defaults_path.exists() else {}
                )
...
def write_consolidated_schemas(components: dict[str, list[dict]]):
    ...
    if "extractors" in components:
        ...
        print(
            f"✓ Wrote {len(extractors_schema)} extractor schemas to schemas/extractors.yaml"
        )
    ...
    if "embedders" in components:
        ...
        print(
            f"✓ Wrote {len(embedders_schema)} embedder schemas to schemas/embedders.yaml"
        )
    ...
    if "stores" in components:
        ...
        with open(SCHEMAS_DIR / "stores.yaml", "w") as f:
            yaml.dump(
                {"stores": stores_schema}, f, default_flow_style=False, sort_keys=False
            )
    ...
    if "retrievers" in components:
        ...
        print(
            f"✓ Wrote {len(retrievers_schema)} retriever schemas to schemas/retrievers.yaml"
        )

    # Write consolidated JSON version
    with open(SCHEMAS_DIR / "consolidated.json", "w") as f:
        json.dump(consolidated_data, f, indent=2)
    print("✓ Wrote consolidated JSON to schemas/consolidated.json")
    ...
    print("✓ Wrote consolidated YAML to schemas/consolidated.yaml")
