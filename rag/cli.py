...
def is_unified_config(config: Dict[str, Any]) -> bool:
    ...
    if (
        ...
        and "rag" in config
        and isinstance(config["rag"], dict)
    ):
        rag_config = config["rag"]
        return (
            "parsers" in rag_config
            and "embedders" in rag_config
            and "vector_stores" in rag_config
            and "defaults" in rag_config
        )
    # Check for legacy v2.0+ format
    return (
        config.get("version", "1.0").startswith("2.")
        and "parsers" in config
        and "defaults" in config
    )


def select_parser_config(
    config: Dict[str, Any], file_type: str, parser_override: Optional[str] = None
) -> Dict[str, Any]:
...
def select_component_config(
    config: Dict[str, Any], component_type: str, override: Optional[str] = None
) -> Dict[str, Any]:
...
def create_pipeline_from_config(
    ...
    parser_config = select_parser_config(
        config, file_type or "unknown", parser_override
    )
    embedder_config = select_component_config(config, "embedders", embedder_override)
    store_config = select_component_config(
        config, "vector_stores", vector_store_override
    )
...
def ingest_command(args):
    ...
    try:
        ...
        if config_extractors:
            ...
            tracker.print_info(
                f"🔧 Config-based extractors found: {', '.join(config_extractor_names)}"
            )
        ...
        if hasattr(args, "extractors") and args.extractors:
            ...
            tracker.print_info(
                f"🔧 CLI extractors specified: {', '.join(cli_extractor_names)}"
            )
                ...
                if hasattr(args, "extractor_config") and args.extractor_config:
                    try:
                        ...
                        tracker.print_warning(
                            f"Invalid CLI extractor config JSON, using defaults: {e}"
                        )

                # Combine config and CLI extractors with CLI taking precedence
                final_extractor_configs = (
                    config_extractors.copy() if config_extractors else {}
                )
                final_extractor_configs.update(cli_extractor_configs)

                # Get all unique extractor names (CLI overrides config)
                all_extractor_names = list(
                    set(config_extractor_names + cli_extractor_names)
                )

                if all_extractor_names:
                    tracker.print_info(
                        f"🔧 Applying {len(all_extractor_names)} extractors: {', '.join(all_extractor_names)}"
                    )

                    # Apply extractors
                    enhanced_documents = apply_extractors_from_cli_args(
                        result.documents, all_extractor_names, final_extractor_configs
                    )
                    result.documents = enhanced_documents
                    tracker.print_success(
                        f"Successfully applied {len(all_extractor_names)} extractors"
                    )
    ...
    try:
        ...
        if retrieval_strategy:
            ...
            retrieval_result = retrieval_strategy.retrieve(
                query_embedding=query_embedding, vector_store=store, top_k=args.top_k
            )
            # Convert RetrievalResult to list of documents with scores
            results = []
            for i, (doc, score) in enumerate(
                zip(retrieval_result.documents, retrieval_result.scores)
            ):
...
def test_command(args):
    ...
    run_basic = not any(
        [
            args.management,
            args.configurations,
            args.retrieval_strategies,
            args.enterprise,
        ]
    )
    if args.all_tests:
        run_basic = args.management = args.configurations = (
            args.retrieval_strategies
...
def run_basic_tests(args, tracker: LlamaProgressTracker) -> Dict[str, Any]:
    ...
    try:
        ...
        results["tests"].append(
            {"name": "Ollama Connection", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
    ...
    try:
        ...
        if hasattr(store, "get_collection_stats"):
            ...
            results["tests"].append(
                {"name": "Enhanced ChromaDB Methods", "status": "PASS"}
            )
            results["passed"] += 1
        ...
        results["tests"].append(
            {"name": "ChromaDB Connection", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
    ...
    try:
        if args.test_file:
            ...
            try:
                ...
                results["tests"].append(
                    {"name": "File Path Resolution", "status": "PASS"}
                )
                results["passed"] += 1
            except FileNotFoundError as e:
                print(f"   ✗ Test file not found: {e}")
                results["tests"].append(
                    {"name": "File Path Resolution", "status": "FAIL", "error": str(e)}
                )
                results["failed"] += 1
                return results
            ...
            if file_extension == ".csv":
                ...
                results["tests"].append(
                    {"name": f"{file_extension} Parser", "status": "SKIP"}
                )
                return results
            ...
            if result.errors:
                ...
                results["tests"].append(
                    {"name": f"{test_type} Error Handling", "status": "WARN"}
                )
                # Show first error for debugging
                if result.errors:
                    print(
                        f"      First error: {result.errors[0].get('error', 'Unknown error')}"
                    )
            else:
                results["tests"].append(
                    {"name": f"{test_type} Error Handling", "status": "PASS"}
                )
                results["passed"] += 1
        else:
        ...
        results["tests"].append(
            {"name": "File Parsing", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
...
def run_management_tests(args, tracker: LlamaProgressTracker) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(
                f"   ✓ Statistics retrieved: {stats.get('total_documents', 0)} documents"
            )
            ...
            results["tests"].append(
                {"name": "Document Statistics", "status": "FAIL", "error": str(e)}
            )
            results["failed"] += 1
        ...
        try:
            ...
            results["tests"].append(
                {"name": "Hash Generation", "status": "FAIL", "error": str(e)}
            )
            results["failed"] += 1
        ...
        try:
            delete_results = doc_manager.deletion_manager.delete_by_time(
                older_than_days=999, strategy=DeletionStrategy.SOFT_DELETE
            )
            ...
            results["tests"].append(
                {"name": "Deletion Manager", "status": "FAIL", "error": str(e)}
            )
            results["failed"] += 1
        ...
        results["tests"].append(
            {"name": "Document Management Setup", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
...
def run_configuration_tests(args, tracker: LlamaProgressTracker) -> Dict[str, Any]:
    ...
    try:
        ...
        if is_unified_config(config):
            ...
            results["tests"].append(
                {"name": "Unified Config Detection", "status": "PASS"}
            )
            results["passed"] += 1
            ...
            if all([parser_config, embedder_config, store_config]):
                ...
                results["tests"].append(
                    {"name": "Component Selection", "status": "PASS"}
                )
                results["passed"] += 1
            else:
                print("   ✗ Component selection failed")
                results["tests"].append(
                    {"name": "Component Selection", "status": "FAIL"}
                )
                results["failed"] += 1
        ...
        if "metadata_config" in str(config):
            ...
            results["tests"].append(
                {"name": "Enhanced Metadata Config", "status": "PASS"}
            )
            results["passed"] += 1
        ...
        results["tests"].append(
            {"name": "Configuration Loading", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
...
def run_retrieval_tests(args, tracker: LlamaProgressTracker) -> Dict[str, Any]:
    ...
    try:
        ...
        results["tests"].append(
            {"name": "Retrieval Strategies", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
...
def run_enterprise_tests(args, tracker: LlamaProgressTracker) -> Dict[str, Any]:
    ...
    try:
        ...
        enterprise_config_path = (
            "config_examples/enterprise_document_management_config.yaml"
        )
        resolver = PathResolver(args.base_dir if hasattr(args, "base_dir") else None)
        ...
        try:
            ...
            results["tests"].append(
                {"name": "Enterprise Config Loading", "status": "PASS"}
            )
            results["passed"] += 1
            ...
            if "enterprise_features" in enterprise_config:
                ...
                results["tests"].append(
                    {"name": "Enterprise Features Config", "status": "PASS"}
                )
                results["passed"] += 1
            ...
            if "compliance" in str(enterprise_config):
                ...
                results["tests"].append(
                    {"name": "Compliance Settings", "status": "PASS"}
                )
                results["passed"] += 1
            ...
            if found_strategies:
                ...
                results["tests"].append(
                    {"name": "Enterprise Retrieval Strategies", "status": "PASS"}
                )
                results["passed"] += 1
            ...
            results["tests"].append(
                {"name": "Enterprise Config Loading", "status": "SKIP"}
            )

    except Exception as e:
        print(f"   ✗ Enterprise feature test failed: {e}")
        results["tests"].append(
            {"name": "Enterprise Features", "status": "FAIL", "error": str(e)}
        )
        results["failed"] += 1
...
def print_test_summary(test_results: Dict[str, Dict], tracker: LlamaProgressTracker):
    ...
    overall_status = (
        "✅ ALL TESTS PASSED"
        if total_failed == 0
        else f"❌ {total_failed} TESTS FAILED"
    )
    if total_skipped > 0:
    ...
    if total_failed == 0:
        ...
        tracker.print_warning(
            f"⚠️  {total_failed} issues found. Please review failed tests."
        )
...
def handle_delete_command(
    args, doc_manager: DocumentManager, tracker: LlamaProgressTracker
):
    ...
    try:
        if args.older_than:
            tracker.print_info(
                f"📅 Deleting documents older than {args.older_than} days"
            )
            results = doc_manager.deletion_manager.delete_by_time(
                older_than_days=args.older_than, strategy=strategy
            )
        elif args.doc_ids:
            tracker.print_info(
                f"🆔 Deleting documents by ID: {', '.join(args.doc_ids)}"
            )
            results = doc_manager.deletion_manager.delete_by_document(
                doc_ids=args.doc_ids, strategy=strategy
            )
        elif args.filenames:
            tracker.print_info(
                f"📄 Deleting documents by filename: {', '.join(args.filenames)}"
            )
            results = doc_manager.deletion_manager.delete_by_filename(
                filenames=args.filenames, strategy=strategy
            )
        elif args.content_hashes:
            tracker.print_info(
                f"🔐 Deleting documents by content hash: {len(args.content_hashes)} hashes"
            )
            results = doc_manager.deletion_manager.delete_by_hash(
                content_hashes=args.content_hashes, strategy=strategy
            )
        elif hasattr(args, "document_hashes") and args.document_hashes:
            tracker.print_info(
                f"📋 Deleting all chunks by document hash: {len(args.document_hashes)} documents"
            )
            results = doc_manager.deletion_manager.delete_by_document_hash(
                document_hashes=args.document_hashes, strategy=strategy
            )
        elif hasattr(args, "source_paths") and args.source_paths:
            tracker.print_info(
                f"📁 Deleting all documents by source path: {len(args.source_paths)} paths"
            )
            results = doc_manager.deletion_manager.delete_by_source_path(
                source_paths=args.source_paths, strategy=strategy
            )
        elif args.expired:
            ...
            if not args.dry_run:
                ...
                if hasattr(doc_manager.vector_store, "delete_collection"):
                    ...
                    tracker.print_success(
                        f"✅ Successfully deleted entire collection ({doc_count} documents)"
                    )
                else:
                ...
                results = {"deleted_count": doc_count, "errors": []}
                tracker.print_info(
                    f"📊 Would delete {doc_count} documents (entire collection)"
                )
        else:
        ...
        if results:
            ...
            if archived_count > 0:
                tracker.print_success(
                    f"📦 Successfully archived {archived_count} documents"
                )
...
def handle_cleanup_command(
    ...
    try:
        if args.old_versions:
            tracker.print_info(
                f"🗂️  Cleaning up old versions (keeping {args.old_versions} latest)"
            )
            results = doc_manager.deletion_manager.cleanup_old_versions(
                keep_versions=args.old_versions
            )
            tracker.print_success(
                f"✅ Cleaned up {results.get('deleted_count', 0)} old versions"
            )
            ...
            results = doc_manager.deletion_manager.delete_expired_documents()
            tracker.print_success(
                f"✅ Cleaned up {results.get('deleted_count', 0)} expired documents"
            )
...
def list_extractors_command(args):
    ...
    print(
        "💡 Test extractors with: uv run python cli.py extractors test --extractor <name>"
    )
...
def test_extractor_command(args):
    ...
    doc = Document(id="test_document", content=text, metadata={"source": "test"})
    ...
    try:
        ...
        if "extractors" in enhanced_doc.metadata:
            ...
            if (
                args.extractor in extractor_data
                or f"{args.extractor}_keywords" in extractor_data
            ):
                # Handle different extractor data structures
                extractor_results = (
                    extractor_data.get(args.extractor)
                    or extractor_data.get(f"{args.extractor}_keywords")
                    or extractor_data.get(f"{args.extractor}_entities")
                    or extractor_data
                )
        ...
        simplified_keys = [
            k
            for k in enhanced_doc.metadata.keys()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simple RAG System CLI with Unified Configuration Support"
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    ingest_parser.add_argument(
        "--embedder",
        help="Override embedder selection (e.g., 'default', 'fast', 'accurate')",
    )
    ingest_parser.add_argument(
        "--vector-store",
    ...
    search_parser.add_argument(
        "--retrieval",
        help="Override retrieval strategy (e.g., 'default', 'filtered', 'hybrid-balanced')",
    )
    search_parser.add_argument(
        "--embedder",
        help="Override embedder selection (e.g., 'default', 'fast', 'accurate')",
    ...
    # Delete commands
    delete_parser = manage_subparsers.add_parser(
        "delete", help="Delete documents with various strategies"
    )
    delete_parser.add_argument(
        "--delete-strategy",
        choices=["soft", "hard", "archive"],
        default="soft",
    ...
    delete_parser.add_argument(
        "--doc-ids", nargs="+", metavar="ID", help="Delete specific document IDs"
    )
    delete_parser.add_argument(
        "--filenames", nargs="+", metavar="FILE", help="Delete documents by filename"
    )
    delete_parser.add_argument(
        "--content-hashes",
        nargs="+",
    ...
    delete_parser.add_argument(
        "--expired", action="store_true", help="Delete expired documents"
    )
    delete_parser.add_argument(
        "--all",
        action="store_true",
    ...
    replace_parser.add_argument(
        "--target-doc-id", required=True, help="Document ID to replace"
    )
    replace_parser.add_argument(
        "--replace-strategy",
        choices=["replace_all", "incremental", "versioning"],
        default="versioning",
        help="Replacement strategy",
    )

    # Stats commands
    stats_parser = manage_subparsers.add_parser(
        "stats", help="Show document statistics"
    )
    ...
    cleanup_parser.add_argument(
        "--expired", action="store_true", help="Clean up expired documents"
    )
    ...
    hash_parser.add_argument(
        "--rehash", action="store_true", help="Regenerate all document hashes"
    )
    ...
    extractor_subparsers = extractor_parser.add_subparsers(
        dest="extractor_command", help="Extractor commands"
    )

    # List extractors
    list_extractors_parser = extractor_subparsers.add_parser(
        "list", help="List available extractors"
    )
    list_extractors_parser.add_argument(
        "--detailed", action="store_true", help="Show detailed extractor information"
    )

    # Test extractor
    test_extractor_parser = extractor_subparsers.add_parser(
        "test", help="Test extractor on text/file"
    )
    test_extractor_parser.add_argument(
        "--extractor", required=True, help="Extractor name to test"
    )
    test_extractor_parser.add_argument("--file", help="File to extract from")
    test_extractor_parser.add_argument("--text", help="Text to extract from")
    test_extractor_parser.add_argument("--config", help="JSON config for extractor")

    # Add extractor arguments to ingest command
    ingest_parser.add_argument(
        "--extractors", nargs="+", help="Extractors to apply (e.g., rake yake entities)"
    )
    ingest_parser.add_argument(
        "--extractor-config",
