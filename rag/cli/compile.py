...
def discover_components(base_path: str, component_type: str) -> list[dict[str, Any]]:
    ...
    if not components_path.exists():
        console.print(
            f"[yellow]No {component_type}s_new directory found at {components_path}[/yellow]"
        )
        return components
            ...
            if schema_file.exists():
                try:
                    ...
                    component_info = {
                        ...
                        "defaults": defaults,
                        "has_implementation": (
                            component_dir / f"{component_dir.name}.py"
                        ).exists(),
                    }
...
def generate_strategy_recommendations(
    extractors: list[dict], parsers: list[dict]
) -> dict[str, Any]:
...
def print_component_summary(components: list[dict[str, Any]], component_type: str):
    ...
    table = Table(
        title=f"Discovered {component_type.title()}s",
        show_header=True,
...
def compile_components(
    base_path: str = ".", output_dir: str = "./schemas", verbose: bool = False
):
    ...
    console.print(
        Panel("[bold blue]RAG Component Schema Compiler[/bold blue]", expand=False)
    )
    ...
    strategies = generate_strategy_recommendations(
        all_components.get("extractor", []), all_components.get("parser", [])
    )
...
def main():
    ...
    parser.add_argument(
        "--base-path", default=".", help="Base path to search for components"
    )
    parser.add_argument(
        "--output-dir",
        default="./schemas",
        help="Output directory for compiled schemas",
    )
