...
Data Error,Database showing wrong values,Bug,high"""
    ...
    assert doc.metadata.get("parser_type") == "CSVParser_Pandas"
...
Data Error,Database showing wrong values,Bug,high"""
    ...
    # Check document content
    doc = result.documents[0]
    assert doc.metadata.get("parser_type") == "CSVParser_Python"
...
"""
    ...
    assert doc.metadata.get("parser_type") == "MarkdownParser_Python"
...
Final paragraph with conclusion."""
    ...
    assert doc.metadata.get("parser_type") == "TextParser_Python"
...
def test_parser_factory():
    ...
    # Test getting a PDF parser
    pdf_parser = factory.get_parser("PDFParser_PyPDF2")
    assert pdf_parser is not None
    assert pdf_parser.__class__.__name__ == "PDFParser_PyPDF2"
    
    # Test getting a CSV parser
    csv_parser = factory.get_parser("CSVParser_Python")
    ...
    assert "PDFParser_PyPDF2" in available
    assert "CSVParser_Python" in available
    assert "CSVParser_Pandas" in available
...
def test_parser_registry():
    ...
    pdf_info = registry.get_parser_info("PDFParser_PyPDF2")
    assert pdf_info is not None
    assert pdf_info["category"] == "pdf"
    
    # Test getting parsers by category
    csv_parsers = registry.get_parsers_by_category("csv")
    assert len(csv_parsers) >= 2  # Should have Python and Pandas versions
    
    # Test getting parser by file extension
    pdf_parsers = registry.get_parsers_for_extension(".pdf")
    assert len(pdf_parsers) >= 1
    assert "PDFParser_PyPDF2" in pdf_parsers
    
    csv_parsers = registry.get_parsers_for_extension(".csv")
    assert len(csv_parsers) >= 2
    
    # Test best parser selection
    best_pdf = registry.get_best_parser(".pdf")
    assert best_pdf in ["PDFParser_PyPDF2"]
    
    best_csv = registry.get_best_parser(".csv")
    assert best_csv in ["CSVParser_Pandas", "CSVParser_Python"]