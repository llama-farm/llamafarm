"""
Test Celery file:// URL construction on Windows and Unix platforms.
...
def test_windows_file_url_construction():
    ...
    # Convert as done in celery_app.py
...
def test_celery_app_loads_on_windows():
    """Test that the celery_app module can be imported on Windows without errors."""
    ...
    with patch.dict(os.environ, {"LF_DATA_DIR": r"C:\Users\test\.llamafarm"}):
        try:
            ...
            if "Port could not be cast" in str(e):
                pytest.fail(f"Celery app failed to load due to malformed URL: {e}")
            raise
