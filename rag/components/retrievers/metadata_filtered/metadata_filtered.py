...
class MetadataFilteredStrategy(RetrievalStrategy):
    ...
    def __init__(
        ...
        self.fallback_multiplier = config.get(
            "fallback_multiplier", 3
        )  # Get 3x more docs when filtering
        ...
        **kwargs,
        ...
        if hasattr(vector_store, "search_with_filter") and filters:
            ...
            documents = vector_store.search_with_filter(
                query_embedding=query_embedding, top_k=top_k, metadata_filter=filters
            )
            ...
            documents = vector_store.search(
                query_embedding=query_embedding, top_k=search_k
            )
