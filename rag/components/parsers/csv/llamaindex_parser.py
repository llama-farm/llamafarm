...
class CSVParser_LlamaIndex:
    ...
    def __init__(self, name: str = "CSVParser_LlamaIndex", config: Optional[Dict[str, Any]] = None):
        self.name = name
    ...
    def parse(self, source: str, **kwargs):
        ...
        try:
            ...
            return ProcessingResult(
                documents=[],
                errors=[{"error": "LlamaIndex or pandas not installed. Install with: pip install llama-index pandas", "source": source}]
            )
                    ...
                    for chunk_idx in range(0, len(df), self.chunk_size):
                        ...
                        if self.combine_content:
                            ...
                            for _, row in chunk_df.iterrows():
                                ...
                                if row_content:
                                    content_parts.append(self.content_separator.join(row_content))
                            content = "\n".join(content_parts)
                        else:
                            ...
                            for row_idx, row in chunk_df.iterrows():
                                ...
                                doc_id = str(row[self.id_field]) if self.id_field and self.id_field in row else f"{path.stem}_row_{row_idx}"
