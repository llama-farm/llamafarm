...
class TextParser_LlamaIndex:
    ...
    def __init__(self, name: str = "TextParser_LlamaIndex", config: Optional[Dict[str, Any]] = None):
        self.name = name
        ...
        self.semantic_breakpoint_percentile_threshold = self.config.get("semantic_breakpoint_percentile_threshold", 95)
    ...
    def parse(self, source: str, **kwargs):
        ...
        try:
            ...
            llama_doc = LlamaDocument(text=text, metadata={"source": str(path), "file_name": path.name})
            ...
            for i, node in enumerate(nodes):
                ...
                if hasattr(node, 'relationships') and node.relationships:
                    ...
                    for rel_type, rel_node in node.relationships.items():
                        relationships[str(rel_type)] = rel_node.node_id if hasattr(rel_node, 'node_id') else str(rel_node)
    ...
    def _is_code_file(self, extension: str) -> bool:
        ...
        code_extensions = {
            ...
            '.sh', '.bash', '.zsh', '.ps1', '.bat', '.jsx', '.tsx', '.vue',
            '.html', '.css', '.scss', '.less', '.yaml', '.yml', '.json',
    ...
    def _get_tokenizer_fn(self):
        ...
        try:
            ...
            logger.warning("tiktoken not available, falling back to simple word tokenizer")
            return lambda text: text.split()
    
    def _extract_metadata(self, path: Path, text: str, is_code_file: bool) -> Dict[str, Any]:
    ...
    def _extract_code_metadata(self, text: str, extension: str) -> Dict[str, Any]:
        ...
        function_patterns = {
            '.py': ['def '],
            '.js': ['function ', 'const ', 'let ', '=>'], '.jsx': ['function ', 'const ', 'let ', '=>'],
            '.ts': ['function ', 'const ', 'let ', '=>'], '.tsx': ['function ', 'const ', 'let ', '=>'],
            '.java': ['public ', 'private ', 'protected '],
            '.cpp': ['int ', 'void ', 'bool '], '.c': ['int ', 'void ', 'bool '],
