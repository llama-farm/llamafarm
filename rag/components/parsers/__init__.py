    ...
    def create_parser(cls, name: str, config: dict = None):
        ...
        if ToolAwareParserFactory:
            try:
                ...
                if "llama-index" in str(e).lower() or "llama_index" in str(e).lower():
                    logger.warning("LlamaIndex not installed. Install with: uv pip install llama-index")
                elif "magic" in str(e).lower():
                    logger.warning("python-magic not installed. Install with: uv pip install python-magic")
                else:
                    logger.debug(f"Import error in ToolAwareParserFactory for {name}: {e}")
            ...
            try:
                ...
                if parser_info:
                    ...
                    if parser_class:
                        ...
                        module_name = parser_info.get('module', f'components.parsers.{name}')
                        try:
                            import importlib
        ...
        try:
            ...
            if module_path.exists():
                spec = importlib.util.spec_from_file_location(f"parser_{name}", module_path)
        ...
        if hasattr(self.config, 'get') and 'parsers' in self.config:
            ...
            if parsers_list and extension in ['.txt', '.text', '.log']:
                ...
                try:
                    ...
                    logger.warning(f"Failed to create default parser {parser_type}: {e}")
...
for legacy_name in legacy_parser_mapping:
    ParserFactory.register_parser(legacy_name, lambda config, n=legacy_name: ParserFactory.create_parser(legacy_parser_mapping[n], config))
