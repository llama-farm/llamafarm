...
class ExcelParser_LlamaIndex:
    ...
    def __init__(self, name: str = "ExcelParser_LlamaIndex", config: Optional[Dict[str, Any]] = None):
        self.name = name
    ...
    def parse(self, source: str, **kwargs):
        ...
        try:
            ...
            return ProcessingResult(
                documents=[],
                errors=[{"error": "LlamaIndex or pandas not installed. Install with: pip install llama-index pandas openpyxl", "source": source}]
            )
        ...
        try:
            ...
            for sheet_name in sheets_to_process:
                ...
                if self.extract_metadata:
                    ...
                    sheet_metadata["dtypes"] = {col: str(dtype) for col, dtype in df.dtypes.items()}
                ...
                if self.chunk_strategy == "rows" and self.chunk_size:
                    ...
                    for chunk_idx in range(0, len(df), self.chunk_size):
                        ...
                        if self.combine_sheets:
                            all_sheet_content.append(f"=== Sheet: {sheet_name} (Rows {chunk_idx}-{min(chunk_idx + self.chunk_size, len(df))}) ===\n{chunk_content}")
                        else:
