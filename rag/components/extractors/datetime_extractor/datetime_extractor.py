...
class DateTimeExtractor(BaseExtractor):
    ...
    def extract(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in documents:
            try:
                ...
                doc.metadata["dates"] = [
                    dt["text"] for dt in datetime_data.get("dates", [])
                ]
                doc.metadata["times"] = [
                    dt["text"] for dt in datetime_data.get("times", [])
                ]
                doc.metadata["relative_dates"] = [
                    dt["text"] for dt in datetime_data.get("relative_dates", [])
                ]

                total_extracted = (
                    len(datetime_data.get("dates", []))
                    + len(datetime_data.get("times", []))
                    + len(datetime_data.get("relative_dates", []))
                )

                self.logger.debug(
                    f"Extracted {total_extracted} datetime elements from document {doc.id}"
                )

            except Exception as e:
                self.logger.error(
                    f"DateTime extraction failed for document {doc.id}: {e}"
                )
        ...
        for sentence in sentences:
            ...
            try:
                ...
                parsed_date, parse_info = self.dateutil_parser.parse(
                    sentence.strip(), fuzzy_with_tokens=True
                )
        ...
        if "am" in time_text.lower() or "pm" in time_text.lower():
            try:
                if self.dateutil_available:
                    ...
                    time_part = re.sub(
                        r"\s*[ap]m\s*", "", time_text, flags=re.IGNORECASE
                    )
    ...
    def _interpret_relative_date(self, relative_text: str) -> str:
        ...
        directions = re.findall(
            r"(?:ago|from\s+now|later|before|after)", relative_text, re.IGNORECASE
        )
    ...
    def _deduplicate_extractions(
        ...
        for extraction in all_extractions:
            ...
            for existing in filtered_extractions:
                if (
                    extraction["start"] < existing["end"]
                    and extraction["end"] > existing["start"]
                ):
