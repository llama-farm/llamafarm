...
class SummaryExtractor(BaseExtractor):
    ...
    def __init__(
        ...
        self.min_sentence_length = (
            config.get("min_sentence_length", 10) if config else 10
        )
        self.max_sentence_length = (
            config.get("max_sentence_length", 500) if config else 500
        )
        self.include_key_phrases = (
            config.get("include_key_phrases", True) if config else True
        )
        self.include_statistics = (
            config.get("include_statistics", True) if config else True
        )
    ...
    def extract(self, documents: List["Document"]) -> List["Document"]:
        ...
        for doc in documents:
            try:
                ...
                self.logger.error(
                    f"Summary extraction failed for document {doc.id}: {e}"
                )
    ...
    def _split_sentences(self, text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if (
                len(sentence) >= self.min_sentence_length
    ...
    def _select_summary_sentences(
        ...
        selected_indices = sorted(
            [idx for idx, score in sorted_sentences[: self.summary_sentences]]
        )
    ...
    def _extract_key_phrases(self, text: str) -> List[str]:
        ...
        for i in range(len(words) - 2):
            ...
            if not any(
                ...
                for word in [words[i], words[i + 1], words[i + 2]]
            ):
                phrases.append(phrase)
    ...
    def _calculate_text_statistics(
        ...
        if sentences:
            ...
            stats["avg_sentence_complexity"] = sum(sentence_lengths) / len(
                sentence_lengths
            )
