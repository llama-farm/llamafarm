...
class LinkExtractor(BaseExtractor):
    ...
    def _extract_from_text(
        ...
        """
        try:
            results = {"extractor": "LinkExtractor"}
            ...
            if self.extract_emails:
                ...
                results["email_domains"] = list(
                    set(email.split("@")[1] for email in emails if "@" in email)
                )
    ...
    def _extract_mentions(self, text: str) -> List[str]:
        ...
        for match in self.mention_pattern.finditer(text):
            ...
            if (
                len(mention) > 1 and mention not in mentions
            ):  # Avoid duplicates and single @
    ...
    def _extract_hashtags(self, text: str) -> List[str]:
        ...
        for match in self.hashtag_pattern.finditer(text):
            ...
            if (
                len(hashtag) > 1 and hashtag not in hashtags
            ):  # Avoid duplicates and single #
