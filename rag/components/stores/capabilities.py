...
class DatabaseCapabilities:
    ...
    DATABASE_CAPABILITIES = {
        ...
        "FAISSStore": {
            "supported": ["basic_similarity", "batch_operations", "distance_metrics"],
    ...
    def get_supported_strategies(cls, database_type: str) -> dict[str, dict[str, Any]]:
        ...
        for strategy_name, strategy_info in cls.STRATEGY_REQUIREMENTS.items():
            ...
            if (
                "database_specific" in strategy_info
                and strategy_info["database_specific"] != database_type
            ):
                continue
    ...
    def get_optimal_strategy(cls, database_type: str, use_case: str = "general") -> str:
        ...
        preferred_order = use_case_preferences.get(
            use_case, use_case_preferences["general"]
        )
        ...
        best_strategy = max(
            strategies.items(), key=lambda x: x[1]["compatibility_score"]
        )
        return best_strategy[0]

    @classmethod
    def validate_configuration(
        cls, database_type: str, strategy_name: str, config: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def get_database_info(cls, database_type: str) -> dict[str, Any]:
        ...
        db_info["supported_strategies"] = list(
            cls.get_supported_strategies(database_type).keys()
        )
