    ...
    if Path(excel_path).exists():
        ...
        for doc in result.documents:
            if "formulas" in doc.metadata and doc.metadata["formulas"]:
                print(
                    f"  Found formulas in sheet '{doc.metadata.get('sheet_name', 'unknown')}'"
                )
                break
    else:
        ...
        if Path(file_path).exists():
            ...
            if result.documents:
                ...
                if "prev_chunk_id" in doc.metadata or "next_chunk_id" in doc.metadata:
                    print("  Chunk relationships preserved")
        else:
...
def compare_parsers():
    ...
    traditional_parser = ToolAwareParserFactory.create_parser(
        ...
        config={"chunk_size": 1200, "chunk_overlap": 150, "extract_metadata": True},
    )
    ...
    traditional_metadata = (
        traditional_result.documents[0].metadata if traditional_result.documents else {}
    )
    llamaindex_metadata = (
        llamaindex_result.documents[0].metadata if llamaindex_result.documents else {}
    )
