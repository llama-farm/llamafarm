...
#   timestamp: 2025-10-15T19:59:19+00:00
...
class LlamaFarmConfig(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        """Validate prompt names are unique and model.prompts reference existing sets."""
        ...
        if self.prompts and self.runtime and hasattr(self.runtime, 'models') and self.runtime.models:
            prompt_names_set = {p.name for p in self.prompts}
            for model in self.runtime.models:
                if hasattr(model, 'prompts') and model.prompts:
                    for prompt_ref in model.prompts:
                        if prompt_ref not in prompt_names_set:
                            raise ValueError(
                                f"Model '{model.name}' references non-existent prompt set '{prompt_ref}'. "
