...
#   timestamp: 2025-10-02T01:26:01+00:00
...
class Lemonade(BaseModel):
    backend: Optional[Backend] = Field(
        "onnx",
        description="Inference backend for Lemonade. onnx (default, works on all systems), llamacpp (CPU/GGUF optimized), or transformers (PyTorch/HuggingFace)",
    )
    port: Optional[int] = Field(11534, description="Port for Lemonade server")
    context_size: Optional[int] = Field(
        32768, description="Context window size in tokens (default 32768 for most models)"
    )
    model_path: Optional[str] = Field(
        None, description="Custom model storage location for Lemonade"
    )
...
class Runtime(BaseModel):
    ...
    huggingface_token: Optional[str] = Field(
        None,
        description="Hugging Face API token for downloading gated models (required for some Lemonade models)",
    )
    ...
    prompt_format: Optional[PromptFormat] = Field(
        None,
        description="Selects prompt formatting pipeline (structured uses Instructor; unstructured sends freeform prompts)",
    )
