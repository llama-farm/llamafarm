...
#   timestamp: 2025-10-16T02:10:30+00:00
...
class Transformers(BaseModel):
    device: Optional[Device] = Field("auto", description="Device to use for model inference")
    torch_dtype: Optional[TorchDtype] = Field("auto", description="PyTorch dtype for model weights")
    trust_remote_code: Optional[bool] = Field(
        True, description="Allow loading models with custom code from HuggingFace"
    )
    model_type: Optional[ModelType] = Field(
        None, description="Type of model (text generation or image generation)"
    )


class DefaultSize(Enum):
    field_256x256 = "256x256"
    field_512x512 = "512x512"
    field_768x768 = "768x768"
    field_1024x1024 = "1024x1024"
    field_1280x1280 = "1280x1280"
...
class Diffusion(BaseModel):
    default_steps: Optional[conint(ge=1, le=150)] = Field(
        50, description="Default number of denoising steps"
    )
    default_guidance: Optional[confloat(ge=1.0, le=20.0)] = Field(
        7.5, description="Default guidance scale (how closely to follow the prompt)"
    )
    default_size: Optional[DefaultSize] = Field("1024x1024", description="Default image size")
    scheduler: Optional[Scheduler] = Field("auto", description="Diffusion scheduler to use")
    enable_optimizations: Optional[bool] = Field(
        True,
        description="Enable platform-specific optimizations (attention slicing, xformers, etc.)",
    )
...
class Model(BaseModel):
    ...
    transformers: Optional[Transformers] = Field(
        None,
        description="Transformers runtime specific configuration (when provider is 'transformers')",
    )
    diffusion: Optional[Diffusion] = Field(
        None, description="Diffusion model configuration (for image generation models)"
    )
