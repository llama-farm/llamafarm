...
#   timestamp: 2025-10-06T16:24:38+00:00
...
class Lemonade(BaseModel):
    ...
    auto_download: Optional[bool] = Field(
        False, description="Automatically download model from HuggingFace if not cached"
    )
    checkpoint: Optional[str] = Field(
        None,
        description="Full HuggingFace checkpoint path including quantization (e.g., 'bartowski/Llama-3.2-1B-Instruct-GGUF:Q4_K_M')",
    )
    recipe: Optional[Recipe] = Field(
        None, description="Lemonade recipe to use with the model (should match backend)"
    )
