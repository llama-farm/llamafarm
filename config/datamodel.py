...
#   timestamp: 2025-09-11T13:06:02+00:00
...
class DirectoryConfig(BaseModel):
    ...
    recursive: Optional[bool] = Field(True, description="Process subdirectories recursively")
    supported_files: Optional[list[str]] = Field(
        ["*"],
        description="Glob patterns for accepted files (recommended)",
        examples=[
            ["*.pdf", "*.PDF"],
            ["*.txt", "*.log"],
            ["*.testdata.csv"],
    ...
    exclude_patterns: Optional[list[str]] = Field(
        ...
        examples=[[".*", "__pycache__/*"], ["*.tmp", "*.bak"]],
    )
    include_patterns: Optional[list[str]] = Field(
        ["*"], description="[DEPRECATED - use supported_files] File patterns to include"
    )
    allowed_mime_types: Optional[list[str]] = Field(
        [], description="[DEPRECATED - use supported_files] MIME types to allow"
    )
    allowed_extensions: Optional[list[constr(pattern=r"^\.[a-zA-Z0-9]+$")]] = Field(
        [], description="[DEPRECATED - use supported_files] File extensions to allow"
    )
    max_files: Optional[conint(ge=1, le=10000)] = Field(
        1000, description="Maximum number of files to process"
    )
    follow_symlinks: Optional[bool] = Field(
        False, description="Follow symbolic links when traversing directories"
    )
...
class DataProcessingStrategy(BaseModel):
    ...
    directory_config: Optional[DirectoryConfig] = Field(
        None,
        description="Configuration for processing directories of files (always-on)",
...
class Dataset(BaseModel):
    ...
    data_processing_strategy: Optional[str] = Field(
        None, description="RAG data processing strategy to use for the dataset"
    )
    database: Optional[str] = Field(None, description="RAG database to use for the dataset")
